...
data_processed <- read.csv("../data/processed/data_processed.csv") %>%
  mutate(data_collection_site = dplyr::recode(data_collection_site,
                                              ...
                                              "Olsen and Fritzlen" = "Olsen",
                                              "Smith and Douglas" = "Douglas",
                                              "Stahl Bading Aust Heycke and Thomasius" = "Stahl",
                                              "Unkelbach and Hogden" = "Unkelbach",
                                              "Vadillo" = "Vadillo"))

# filter out data from sites that did not meet the preregistered minimum sample size
sites_meeting_minimum_n <- data_processed %>%
  filter(simulated_data == FALSE) %>%
  count(data_collection_site) %>%
  
  # manual modifications to these participant counts based on participants run who do not have data files
  mutate(n_run = ifelse(data_collection_site == "Moran Hussey and Hughes", n + 1, # one particpant's files were manually excluded based on missing data on the open ended response questions. this was diagnosed by the processing script not running when their files were included. 
                    ifelse(data_collection_site == "Corneille and Mierop", n + 1, # one particpant's files were manually excluded based on missing data on the open ended response questions. this was diagnosed by the processing script not running when their files were included. 
                           n))) %>%
  # exclude sites that didn't reach the preregistered minimum n
  filter(n_run >= 100)
  
data_processed_sites_meeting_minimum_n <- data_processed %>%
  semi_join(sites_meeting_minimum_n, by = "data_collection_site")
...
data_processed_sites_meeting_minimum_n %>%
  summarize(n = n(),
...
Exploratory analysis not include in prereg, as far as I know.

Awareness exclusion rate after doing surveillance exclusions. 
...
exclusion_rate_per_site <- data_processed_sites_meeting_minimum_n %>%
  filter(exclude_surveillance == FALSE) %>%
...
data_1 <- data_processed_sites_meeting_minimum_n %>%
...
data_2 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE &  # exclude if accuracy on surveillance task is >3SD worse than group
...
data_3 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE &  # exclude if accuracy on surveillance task is >3SD worse than group
...
data_4 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE &  # exclude if accuracy on surveillance task is >3SD worse than group
...
data_5 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE) %>% # exclude if accuracy on surveillance task is >3SD worse than group
  mutate(DV = sum_score_evaluation_CSpos_preferred,
...
data_6 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE) %>% # exclude if accuracy on surveillance task is >3SD worse than group
  mutate(DV = sum_score_evaluation_CSpos_preferred,
...
data_7 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE) %>% # exclude if accuracy on surveillance task is >3SD worse than group
  mutate(DV = sum_score_evaluation_CSpos_preferred,
...
data_8 <- data_processed_sites_meeting_minimum_n %>%
  filter(simulated_data == FALSE &  # exclude simulated data used for script development
           exclude_surveillance == FALSE) %>% # exclude if accuracy on surveillance task is >3SD worse than group
  mutate(DV = sum_score_evaluation_CSpos_preferred,
