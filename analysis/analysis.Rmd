...
Stahl pointed out that analysis 9, the model technically assumes independence of the exclusion criteria and that they violate these assumptions given they are nested. We therefore construct a model that acknowledges this non-independence. However, prior to doing so we note that the power to detect moderation in this new model will be at best equal to the power of the original model given that additional parameters are estimated in this new model. We therefore expected no changes to conclusions. 
...
data_effect_sizes_10 <- data_10 %>%
  ...
  dplyr::summarize(preference_mean = mean(DV),
                   preference_sd = sd(DV),
                   preference_n = n()) %>%
  mutate(criterion_type = ifelse(str_detect(awareness, "O&F"), "O&F",
  ...
  dplyr::mutate(preference_cohens_dz = preference_mean/preference_sd,
                ...
                hedges_g_se = sqrt(hedges_g_V)) %>%
  ungroup() %>%
  dplyr::select(data_collection_site, awareness, criterion_type, hedges_g, hedges_g_se) %>%
  mutate(awareness = fct_relevel(awareness, "O&F", "O&F modified", "BA,DeH,&N", "BA,DeH,&N modified"),
         criterion_type = fct_relevel(criterion_type, "O&F", "BA,DeH,&N"))
...
results_10 <- 
  rma.mv(yi  = hedges_g, 
         V   = hedges_g_se^2,
         mods = ~ awareness,
         random =  ~1 | data_collection_site/criterion_type,
         data = data_effect_sizes_10,
         slab = paste(data_collection_site, criterion_type))
