  ...
  if (plot == TRUE) {
    ...
    combined_plotting_data <- data_effect_sizes %>%
      ...
      mutate(Site      = fct_rev(data_collection_site),
             # set point size to proportionate range from 2-6
             size             = 6 - (hedges_g_se - min(hedges_g_se)) / (max(hedges_g_se) - min(hedges_g_se)) * 4,
...
data_1 <- data_processed %>%
  filter(exclude_surveillance == FALSE &
           ...
           exclude_aware_olsen_and_fazio == FALSE) %>%
...
data_2 <- data_processed %>%
  filter(exclude_surveillance == FALSE &
         simulated_data == FALSE &
         exclude_awareness_baranan_dehouwer_nosek == FALSE) %>%
  mutate(DV = sum_score_evaluation_CS1_preferred)
...
data_3 <- data_processed %>%
  filter(exclude_surveillance == FALSE & 
           simulated_data == FALSE &
           exclude_awareness_baranan_dehouwer_nosek_modified == FALSE) %>%
  mutate(DV = sum_score_evaluation_CS1_preferred)
...
## Exclusion strategy 4: XXX strategy
