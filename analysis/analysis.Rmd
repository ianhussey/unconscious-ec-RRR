...
# helper function to apply to each resample to calculate hedges g -----
moderator_preferences_effect_size <- function(split){
...
# moderator meta analysis and forest plot workflow -----
moderator_meta_analysis_workflow <- function(data, 
  ...
  data_effect_sizes <- data %>%
    ...
    bootstraps(times = nboots) %>%
    # apply to each bootstrap
    mutate(es_boots = map(splits, moderator_preferences_effect_size)) %>%
    # unnest
    unnest(es_boots) %>%
    # summarize across resamples
    group_by(data_collection_site, awareness) %>%
  ...
  if (plot == TRUE) {
    ...
    meta_data_temp <- meta_analysis_results %>%
      ...
      select(data_collection_site, awareness, hedges_g, hedges_g_se, hedges_g_ci_lwr, hedges_g_ci_upr)
...
## Analysis 1 (Confirmatory): Olson & Fazio (2002) exclusion criteria
...
results_1$meta_analysis_results
...
## Analysis 5 (Exploratory): Olson & Fazio (2002) exclusion criteria as a moderator

Refit each of the above using the analysis-specific exclusion criterion (e.g., olsen and fazio) as a moderator instead.
...
- Moderator meta anaysis results: `r results_5$moderator_test_results_text` 

## Analysis 6 (Exploratory): Bar-Anan, De-Houwer & Nosek (2010)'s exclusion criteria as a moderator
...
- Moderator meta anaysis results: `r results_6$moderator_test_results_text` 
...
- Moderator meta anaysis results: `r results_7$moderator_test_results_text` 
...
- Moderator meta anaysis results: `r results_8$moderator_test_results_text` 
